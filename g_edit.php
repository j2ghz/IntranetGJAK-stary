...
if(Prihlasen3($kod, $REMOTE_ADDR, $skupina, 4, $fullname, $login, $chyba))
{
  ...
  if(!($vyber)) $vyber=1;
  Hlavicka("Aktualizace souborù pro studenty", $fullname, $kod, "g_edit.php", $pole_vyberu, $pole_tlacitek, $vyber);
  echo "<P>";
  ...
  if($odeslano_vymaz)
  {
    ...
    $SQL = "select nazev from soubory where $podminka ";
      if(DB_select($SQL, $vystup, $pocet))
      {
        ...
        {
          if(!(unlink(c_files."files_gym/".$zaznam["nazev"])))
  ...
  {
    case 1:
      //Tlacitka($kod, "g_edit.php", $pole_vyberu, $pole_tlacitek,1);
/*******************************************************/
      echo "<center>".Hlaska($chyba, "Soubor se nepodaøilo ulo¾it", "Soubor byl úspì¹nì ulo¾en")."</center>";
      if($chyba<>"ok")
      {
        echo "<form action=\"./g_send.php?kod=$kod\" method=post enctype=\"multipart/form-data\">";
        ...
        $SQL = "select * from typ where skola='g' order by id";
      ...
      break;
    case 2:
      //Tlacitka($kod, "g_edit.php", $pole_vyberu, $pole_tlacitek,2);
/*******************************************************/
      if(!($typ)) $typ="g_predpisy";
      $SQL = "select * from typ where skola='g' order by id";
      if(DB_select($SQL, $vystup, $pocet))
      {
      ...
      if(DB_select($SQL, $vystup, $pocet))
      {
        if($pocet>0)
        {
          ...
          while($zaz=MySQL_fetch_array($vystup))
          {
            ...
            echo "<td><a href=\"sendfile.php?kod=$kod&p_prava=5&p_nazev=".$zaz["nazev"]."&p_adresar=files_gym/".$zaz["nazev"]."\">".$zaz["popis"]."</a></td>";
